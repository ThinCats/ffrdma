...
int RDMA_Allreduce_exp(void *sendbuf, void *recvbuf,
                       int count, int datatype, int op, RDMA_Comm comm)
{
    ...
    count *= type_static[sendtype];

    auto send_msg = AMessage_create((void *)sendbuf, count, 0);
    ...
    memcpy((unsigned char *)recvbuf, sendbuf, count);
    ...
    for (int i = 2; i < whole_ranks; i++)
    {
        ...
        {
            double *resbuf = (double *)recvbuf;
            double *msgbuffer = (double *)recv_msg->buffer;
            if (op == 0)
            {
                for (int j = 0; j < count / 8; j++)
                {
